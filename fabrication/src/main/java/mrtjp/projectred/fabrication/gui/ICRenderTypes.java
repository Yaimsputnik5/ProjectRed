...
public class ICRenderTypes {
    ...
    public static void renderTextCenteredAt(PoseStack stack, Vector3 pos, String text, int bgColor, int textColor) {
        ...
        fontRenderer.drawInBatch(text, (float) (-fontRenderer.width(text) / 2), (float) (-fontRenderer.lineHeight / 2), textColor, false, stack.last().pose(), buffer, Font.DisplayMode.NORMAL, bgColor, 15728880);
        buffer.endBatch();
    ...
    public static void onTextureStitchEvent(TextureStitchEvent.Post event) {
        ...
        icSurfaceIcon = event.getAtlas().getSprite(new ResourceLocation(MOD_ID, "block/perfboard"));
        icSurfaceBorderIcon = event.getAtlas().getSprite(new ResourceLocation(MOD_ID, "block/perfboard_edge"));
        icSurfaceCornerIcon = event.getAtlas().getSprite(new ResourceLocation(MOD_ID, "block/perfboard_corner"));
    }
}
